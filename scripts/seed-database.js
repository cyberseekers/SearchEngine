...
const run = async () => {
  ...
  const normalUser = await createSpecificUser("normalUser", "abc123");
  const adminUser = await createSpecificUser("adminUser", "abc123");
  await createFakeAdmin(adminUser.id);
  ...
  console.log(`- Password: ${normalUser.passwordHash}`);
